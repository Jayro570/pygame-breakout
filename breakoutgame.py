...
bg = pygame.image.load("XD2.0")
bg = pygame.transform.scale(bg, size)
bi = pygame.image.load("XD2.0")
...
for i in range(7):
    brick = Brick(LIGHTBLUE,80,20)
    brick.rect.x = 60 + i* 100
    brick.rect.y = 90
    all_sprites_list.add(brick)
    all_bricks.add(brick)
for i in range(7):
    brick = Brick(WHITE,80,20)
    brick.rect.x = 60 + i* 100
    brick.rect.y = 120
    all_sprites_list.add(brick)
    all_bricks.add(brick)
...
while carryOn:
    ...
    # First, clear the screen to light blue.
